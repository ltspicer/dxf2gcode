...
if echo "$answer" | grep -iq "^y" ;then
    ...
    echo "${RED}https://github.com/ltspicer/dxf2gcode${NC} (download as zip and unzip on local disk)"
    echo "Are you ready (y/n)?"
    read answer
    if echo "$answer" | grep -iq "^y" ;then
        echo ""
    ...
    echo "Enter path to the dxf2gcode source in your home directory e.g. Downloads/dxf2gcode-main (without / at the beginning and end!)"
    read SRC
